...
| Shared-signature reproducibility | Releases from 0.1.0-preview.8 are built like the buildserver and checked by the Reproducibility workflow; the recipe sets `Binaries` and `AllowedAPKSigningKeys`. Pin `build.expectedSha256` in `recognition/paddle/source-runtime.lock.json` once CI shows a stable source-built runtime. |
